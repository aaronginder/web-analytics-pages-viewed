...
def main():
    ...
    parser.add_argument(
        "--bootstrap_servers", default="localhost:29092", help="Kafka bootstrap servers"
    )
    parser.add_argument(
        "--topic", default="web-analytics-events", help="Kafka topic name"
    )
    parser.add_argument(
        "--num_events", type=int, default=1000, help="Number of events to produce"
    )
    ...
    for event in generate_synthetic_events(args.num_events):
        producer.send(
            topic=args.topic, value=event, key=event["user_id"].encode("utf-8")
        )
